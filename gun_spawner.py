...
from config import (GUN_SPAWN_INTERVAL, GUN_PICKUP_RADIUS, 
                    GUN_SPAWN_POINTS, DEFAULT_STARTING_WEAPON)
...
class GunSpawner:
    ...
    def initialize_map(self, map_name):
        ...
        for x, y, weapon_id in spawn_data:
            self.active_spawns.append([x, y, weapon_id, 1])  # Start with all active
            
        # Initialize cooldowns (all start at 0)
        self.spawn_cooldowns = [0.0] * len(self.active_spawns)
        
        print(f"[GUN_SPAWNER] Initialized {len(self.active_spawns)} spawn points for {map_name}")
        
    def update(self, delta_time):
        """Update spawn cooldowns and respawn guns"""
        for i in range(len(self.spawn_cooldowns)):
            if self.spawn_cooldowns[i] > 0:
                self.spawn_cooldowns[i] -= delta_time
                if self.spawn_cooldowns[i] <= 0:
                    # Respawn the gun
                    self.spawn_cooldowns[i] = 0
                    self.active_spawns[i][3] = 1  # Set as active
                    
    def check_pickup(self, player_x, player_y):
        """Check if player is near any gun spawn and return weapon_id if pickup possible"""
        for i, spawn in enumerate(self.active_spawns):
            ...
            if is_active == 1:
                ...
                if dist <= self.PICKUP_RADIUS:
                    # Player picked up the gun
                    self.active_spawns[i][3] = 0  # Deactivate spawn
                    self.spawn_cooldowns[i] = self.SPAWN_INTERVAL  # Start cooldown
                    return weapon_id
                    
        return None  # No gun picked up
...
class PlayerInventory:
    ...
    def __init__(self, starting_weapon_id=None):
    ...
    def pickup_gun(self, weapon_id):
        """Pickup a new gun. Returns the dropped weapon_id or None"""
        new_gun = get_weapon(weapon_id)
        
        # If second slot is empty, put gun there
        if self.guns[1] is None:
            self.guns[1] = new_gun
            return None
        else:
            # Replace current gun and drop it
            dropped_id = self.guns[self.current_slot].gun_id
